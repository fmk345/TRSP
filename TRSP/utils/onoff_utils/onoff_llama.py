...
class Scaling_LlamaDecoderLayer(nn.Module):
    def __init__(self, original_decoder_layer,scaling_factor):
        super().__init__()
        self.hidden_size = original_decoder_layer.hidden_size
        self.self_attn = original_decoder_layer.self_attn
        self.mlp = original_decoder_layer.mlp
    ...
    def forward(
        ...
        **kwargs,
        ...
        return outputs*self.scaling_factor
...
def scaling_replace(model):
    num_layers = len(model.model.layers)
    for i in range(num_layers):
        model.model.layers[i] = Scaling_LlamaDecoderLayer(model.model.layers[i],model.scaling_factor[i])
    print("Replacement complete.")
