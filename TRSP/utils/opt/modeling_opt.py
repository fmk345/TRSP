...
from ..output import ScaleBaseModelOutputWithPast
...
@dataclass
class ScaleCausalLMOutputWithPast(ModelOutput):
    """
    ...
    Args:
        ...
        scaling_factors (`List[Tuple[torch.FloatTensor, int]]`):
            List of `Tuple[torch.FloatTensor, int]` of length `config.n_layers`, with each tuple having one `torch.FloatTensor` with shape of `(1,1)` and one `int`
    ...
    scaling_factors: List[Tuple[torch.FloatTensor, int]] = None
...
class ScaleOPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig, layer_idx: int = None):
        ...
        self.scaling_factor = nn.Embedding(1,1)
        self.scaling_factor.weight.data.fill_(1.0000000000000)
    ...
    def forward(
        ...
        if not self.do_layer_norm_before:
            hidden_states = self.final_layer_norm(hidden_states)
        outputs = (hidden_states,)
        l2norm=torch.norm(r1-hidden_states)
        outputs+=(l2norm,)
        if output_attentions:
...
class OPTPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["ScaleOPTDecoderLayer"]
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.layers = nn.ModuleList([ScaleOPTDecoderLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        position_ids: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.Tensor] = None,
    ) -> Union[Tuple, ScaleBaseModelOutputWithPast]:
        r"""
        Args:
            input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if decoder_layer.res_layer and not decoder_layer.pass_layer:
                total_layer_loss+=layer_outputs[1]
            if use_cache:
                next_decoder_cache = layer_outputs[3 if output_attentions else 2]
            if output_attentions:
                all_self_attns += (layer_outputs[2],)
        total_layer_loss=total_layer_loss*0.0001
        ...
        return ScaleBaseModelOutputWithPast(
...
class ScaleOPTForCausalLM(OPTPreTrainedModel, GenerationMixin):
    ...
    @replace_return_docstrings(output_type=ScaleCausalLMOutputWithPast, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        cache_position: Optional[torch.Tensor] = None,
        **kwargs,
    ) -> Union[Tuple, ScaleCausalLMOutputWithPast]:
        r"""
        Args:
            input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
                Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you
        ...
        # scaling_factors=[]
        # for i in alive_list:
        #     if not self.model.decoder.layers[i].pass_layer:
        #         l1_norm+= torch.norm(self.model.decoder.layers[i].scaling_factor.weight, p=1)
        # for i in alive_list_select:
        #     if not self.model.decoder.layers[i].pass_layer:
        #         scaling_factors.append((self.model.decoder.layers[i].scaling_factor.weight,i))
        loss = None
        if labels is not None:
            ...
            loss+=lamda_reg*outputs[1]
        if not return_dict:
            output = (logits,) + outputs[1:]
            return (loss,) + output if loss is not None else output
        # return ScaleCausalLMOutputWithPast(
        #     loss=loss,
        #     logits=logits,
