...
class Scaling_OPTDecoderLayer(nn.Module):
    def __init__(self, original_decoder_layer):
        super().__init__()
        self.embed_dim = original_decoder_layer.embed_dim
        self.self_attn = original_decoder_layer.self_attn
        ...
        self.final_layer_norm = original_decoder_layer.final_layer_norm
        self.pass_layer = False
        # self.scaling_factor_accumulated = nn.Parameter(torch.tensor(0.0))  # 可训练的参数，初始值为0    
    def turn_off(self):
        self.pass_layer = True
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,
        # scaling_factor_accumulated: torch.Tensor=None,  # 外部传递累加值
        attention_mask: Optional[torch.Tensor] = None,
        layer_head_mask: Optional[torch.Tensor] = None,
        past_key_value: Optional[Tuple[torch.Tensor]] = None,
        output_attentions: Optional[bool] = False,
        ...
        if not self.do_layer_norm_before:
            hidden_states = self.final_layer_norm(hidden_states)
        scale = self.scaling_factor.weight
        # scaling_factor_accumulated = scaling_factor_accumulated + scale
        outputs = (hidden_states*scale,)
        if output_attentions:
            outputs += (self_attn_weights,)
...
def scaling_replace(model):
    num_layers = len(model.model.decoder.layers)
    for i in range(num_layers):
        # import pdb
        # pdb.set_trace()
        model.model.decoder.layers[i] = Scaling_OPTDecoderLayer(model.model.decoder.layers[i])
    print("Replacement complete.")
